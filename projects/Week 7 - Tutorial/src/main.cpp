...
int main() {
	...
	if (loadScene) {
		...
		Guid daggerMesh = ResourceManager::CreateMesh("dagger2.obj");
		Guid daggerTex = ResourceManager::CreateTexture("textures/astra-cat.png");
		...
		monkeyMaterial->Shininess = 0.1f;
		scene->Materials[monkeyMaterial->GetGUID()] = monkeyMaterial;

		MaterialInfo::Sptr daggerMaterial = std::make_shared<MaterialInfo>();
		daggerMaterial->Shader = scene->BaseShader;
		daggerMaterial->Texture = ResourceManager::GetTexture(daggerTex);
		daggerMaterial->Shininess = 1.f;
		scene->Materials[daggerMaterial->GetGUID()] = daggerMaterial;
		...
		RenderObject dagger = RenderObject();
		dagger.Position = glm::vec3(0.f, -1.f, 1.f);
		dagger.Mesh     = ResourceManager::GetMesh(daggerMesh);
		dagger.Material = daggerMaterial;
		dagger.Rotation.y = 90.f;
		dagger.Scale.x  = 0.3f;
		dagger.Scale.y = 0.3f;
		dagger.Scale.z = 0.3f;
		dagger.Name = "Dagger";
		scene->Objects.push_back(dagger);
